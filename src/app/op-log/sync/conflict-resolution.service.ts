...
export class ConflictResolutionService {
  ...
  private async _createStopFromDateWinOp(
    ...
    // CRITICAL FIX: When the local side has already deleted the repeat config,
    // getCurrentEntityState returns undefined. In that case, we try to
    // extract the entity from local DELETE ops' payloads so we can
    // populate taskRepeatCfgSnapshot. This fixes legacy HRS stop operations
    // (from older builds) that did not carry their own snapshot.
    const resolvedEntityState =
      entityState ?? this._extractEntityFromLocalDeleteOperation(conflict) ?? undefined;
    const payloadRecord =
    ...
    const mergedActionPayload = {
      ...
      ...(resolvedEntityState && typeof resolvedEntityState === 'object'
        ? { taskRepeatCfgSnapshot: resolvedEntityState }
        : {}),
    };
    const payload =
  ...
  /**
   * Extracts entity state from a local DELETE operation payload.
   *
   * When the local side has already deleted an entity and moved it to Recycle Bin,
   * we need to recover the entity state from the pending local DELETE op's payload
   * for conflict resolution (e.g., to populate taskRepeatCfgSnapshot in a
   * replacement HRS stop operation).
   *
   * @param conflict - The conflict containing local DELETE operations
   * @returns Entity state from the first local DELETE payload found, or undefined
   */
  private _extractEntityFromLocalDeleteOperation(
    conflict: EntityConflict,
  ): unknown | undefined {
    const deleteOp = conflict.localOps.find((op) => op.opType === OpType.Delete);
    if (!deleteOp) {
      return undefined;
    }
  ...
  private _convertToLWWUpdatesIfNeeded(conflict: EntityConflict): Operation[] {
    ...
    // Enrich legacy remote HRS stop operations that lack taskRepeatCfgSnapshot.
    // When the local side has already deleted the repeat config, extract the
    // entity from the local DELETE payload and inject it as the snapshot.
    const enrichedRemoteStopOps = this._enrichRemoteStopOpsWithSnapshotIfNeeded(
      remoteStopOps,
      conflict,
    );

    if (!remoteNonStopOps.length) {
      return enrichedRemoteStopOps;
    }
    ...
    return [...convertedNonStopOps, ...enrichedRemoteStopOps];
  }

  /**
   * Enriches legacy remote HRS stop operations that lack taskRepeatCfgSnapshot.
   *
   * When the local side has already deleted the repeat config (local DELETE op
   * in the conflict), we extract the entity from the local DELETE payload and
   * inject it as the taskRepeatCfgSnapshot in each legacy remote stop that's
   * missing one. This fixes sync issues with HRS stop operations generated by
   * older builds (before .8) that did not include the snapshot.
   */
  private _enrichRemoteStopOpsWithSnapshotIfNeeded(
    stopOps: Operation[],
    conflict: EntityConflict,
  ): Operation[] {
    if (!stopOps.length) return stopOps;

    // Check each stop op to see if any need enrichment
    const needsEnrichment = stopOps.some((op) => {
      const actionPayload = extractActionPayload(op.payload);
      return !actionPayload['taskRepeatCfgSnapshot'];
    });
    if (!needsEnrichment) return stopOps;

    // Try to extract the entity from local DELETE payload
    const snapshotEntity = this._extractEntityFromLocalDeleteOperation(conflict);
    if (!snapshotEntity) return stopOps;

    // Add the snapshot to each stop op that's missing it
    return stopOps.map((op) => {
      const actionPayload = extractActionPayload(op.payload);
      if (actionPayload['taskRepeatCfgSnapshot']) return op;
      ...
      const enrichedActionPayload = {
        ...actionPayload,
        taskRepeatCfgSnapshot: snapshotEntity,
      };
      const enrichedPayload =
        'actionPayload' in payloadRecord
          ? { ...payloadRecord, actionPayload: enrichedActionPayload }
