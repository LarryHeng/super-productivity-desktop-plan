# Super Productivity 桌面计划定制版

这是基于 [Super Productivity](https://github.com/super-productivity/super-productivity)
开发的个人定制分支，保留上游的任务管理、计时与项目功能，并补充了桌面任务小组件、
实际时间表和更适合按周规划的工作流。

> [!IMPORTANT]
> 这不是 Super Productivity 官方发行版。仓库只保存源代码与开发文档，不包含用户数据、
> 备份、上传的背景图片或 Windows 安装包。

## 与上游版本的主要不同

### 桌面任务小组件

- 直接复用应用内的艾森豪威尔矩阵，任务状态、计时和 DONE 操作与主程序实时同步。
- 小组件固定在桌面层，普通软件窗口会覆盖它；使用 Windows“显示桌面”后会自动恢复。
- 支持拖动、缩放、最小尺寸限制以及位置和大小记忆。
- 点击任务开始倒计时，再次点击暂停或继续；时间结束后可以输入延长分钟数。
- 只有 DONE 会完成并置灰任务。每日结算后，已完成任务才会离开矩阵。
- 可单独设置背景图、背景可见度和文字内容可见度；没有单独背景时自动继承全局背景。
- Open 会恢复并最大化主程序；关闭小组件会同步关闭主程序中的对应设置。

### 时间表：实际工作记录

- 原“计划表”改为“时间表”，用于展示已经发生的实际工作，不再把预计时长当作实际记录。
- 周视图固定为周一至周日，历史周可查看，并提供“回到今天”。
- 当前时间线按真实时间移动，当天整列高亮。
- 实际计时块不可拖动或修改；规划产生的参考块仍可拖动，并在与实际工作冲突时自动顺延。
- 同一任务的相邻实际记录可自动合并，合并间隔由用户设置，范围为 0 至 30 分钟。
- 月视图实时汇总每天总工作时长，并按任务合并显示各自用时；点击日期可跳转到对应周。

### 规划表：待做安排

- 规划表继续用于可调整的未来安排，与时间表中的实际记录分开保存和展示。
- 周视图固定为周一至周日，支持浏览历史周并快速返回今天。
- 新增与时间表一致的月视图模板，只枚举每天计划的工作内容，不显示实际时长。
- 点击月视图中的日期会联动打开该日期所在周。

### 空闲记录与每日结算

- 空闲检测弹窗可以选择已有任务或新建任务，并把完整的空闲时间段补记到该任务，
  而不是从确认弹窗的时刻重新开始计时。
- 支持手动“结束这一天”和按逻辑日自动结算；默认每天 04:00 进入新的一天。
- 结算时移除已完成任务的 Urgent 和 Important 标签，不归档任务，因此历史记录仍然保留。

### 背景、备份与窗口行为

- 支持用户上传全局背景图，并独立设置小组件背景；两者都可以重置。
- 背景图丢失或读取失败时会安全回退，不因图片异常导致程序退出。
- 支持选择外部本地备份目录，并自动迁移已有备份；原目录通过兼容链接继续工作。
- 主窗口最小化后保留在任务栏，关闭到托盘仍是独立行为。
